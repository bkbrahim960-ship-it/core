...
export class VidApiProvider extends BaseProvider {
    ...
    private async getSources(
        ...
        for (const apiUrl of this.API_URLS) {
            try {
                const result = await this.tryFetchFromApi(apiUrl, media);
                if (result) return result;
            } catch (e) {
                errors.push(
                    `${apiUrl}: ${e instanceof Error ? e.message : 'error'}`
                );
            }
        }

        return this.emptyResult(
            `All VidAPI endpoints failed: ${errors.join('; ')}`
        );
    }

    private async tryFetchFromApi(
        apiUrl: string,
        media: ProviderMediaObject
    ...
    async healthCheck(): Promise<boolean> {
        const urls = [this.BASE_URL, 'https://vidapi.ru'];
